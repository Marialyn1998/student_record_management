...
class DbFunction{
	...
	function logins($loginid,$password){
	...
	function loginss($loginid,$password){
...
function create_grade($grades,$subject){
		
				if($grades==""){
			 
			echo "<script>alert('Select  Grade')</script>";
			...
			$db = Database::getInstance();
			$mysqli = $db->getConnection();
			$query = "insert into grade(gradeid,subid,subject,grades)values(?,?,?,?)";
			$stmt= $mysqli->prepare($query);
			if(false===$stmt){
				...
				$stmt->bind_param('sssss',$gradeid,$subid,$subject,$grades);
				$stmt->execute();
...
function showinstructor1($instructorid){
...
function create_instructor($instructorid,$cid,$subid,$fullname,$specailization){
		 
				if($fullname==""){
			 
			echo "<script>alert('Selected  instructor Name')</script>";
		
		}
		
		
		else if($specailization==""){
			 
			echo "<script>alert('Selected instructor specialazation')</script>";
			...
			$db = Database::getInstance();
			$mysqli = $db->getConnection();
			$query = "insert into instructor(instructorid,cid,subid,fullname,specailization)values(?,?,?,?,?)";
			$stmt= $mysqli->prepare($query);
			if(false===$stmt){
				...
				$stmt->bind_param('sssss',$instructorid,$cid,$subid,$fullname,$specailization);
				$stmt->execute();
...
function showinstructor_register(){
...
function instructor_register($cshort,$cfull,$fname,$mname,$lname,$gender,$gname,$ocp,$income,$category,$ph,
                  $nation,$mobno,$email,$country,$state,$city,$padd,$cadd,$board1,$board2,$roll1,$roll2,
				   $pyear1,$pyear2,$sub1,$sub2,$marks1,$marks2,$fmarks1,$fmarks2,$session){
...
function del_std($id){
 ...
 function del_grade($id){
    ...
    $stmt->bind_param('i',$id);
	$stmt->execute();
    echo "<script>alert('Grade has been deleted')</script>";
   // echo "<script>window.location.href='view-course.php'</script>";
}


 function del_instructor_register($instructorid){
	...
	$stmt->execute();
    echo "<script>alert('instructor has been deleted')</script>";
   // echo "<script>window.location.href='view-instructor.php'</script>";
}
}
